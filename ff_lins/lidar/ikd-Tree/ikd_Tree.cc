...
template <typename PointType> void KD_TREE<PointType>::release() {
    stop_thread();
    Delete_Storage_Disabled = true;
    delete_tree_nodes(&Root_Node);
    PointVector().swap(PCL_Storage);
...
template <typename PointType> void KD_TREE<PointType>::setDeleteCriterionParam(float delete_param) {
    delete_criterion_param = delete_param;
}

template <typename PointType> void KD_TREE<PointType>::setBalanceCriterionParam(float balance_param) {
    balance_criterion_param = balance_param;
}

template <typename PointType> void KD_TREE<PointType>::setDownsampleParam(float downsample_param) {
...
template <typename PointType> BoxPointType KD_TREE<PointType>::treeRange() {
    BoxPointType range;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
        if (Root_Node != nullptr) {
...
template <typename PointType> void KD_TREE<PointType>::multi_thread_rebuild() {
    ...
    while (!terminated) {
        ...
        if (Rebuild_Ptr != nullptr) {
            ...
            search_flag_mutex.unlock(); // Lock deleted points cache
...
template <typename PointType>
void KD_TREE<PointType>::nearestSearch(PointType point, int k_nearest, PointVector &Nearest_Points,
...
template <typename PointType>
void KD_TREE<PointType>::boxSearch(const BoxPointType &Box_of_Point, PointVector &Storage) {
    Storage.clear();
    Search_by_range(Root_Node, Box_of_Point, Storage);
}

template <typename PointType>
void KD_TREE<PointType>::radiusSearch(PointType point, const float radius, PointVector &Storage) {
    Storage.clear();
    Search_by_radius(Root_Node, point, radius, Storage);
}

template <typename PointType> int KD_TREE<PointType>::addPoints(PointVector &PointToAdd, bool downsample_on) {
...
template <typename PointType> void KD_TREE<PointType>::addPointBoxes(std::vector<BoxPointType> &BoxPoints) {
...
template <typename PointType> void KD_TREE<PointType>::deletePoints(PointVector &PointToDel) {
...
template <typename PointType> int KD_TREE<PointType>::deletePointBoxes(std::vector<BoxPointType> &BoxPoints) {
    int tmp_counter = 0;
    for (int i = 0; i < BoxPoints.size(); i++) {
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
...
template <typename PointType> void KD_TREE<PointType>::acquireRemovedPoints(PointVector &removed_points) {
    points_deleted_rebuild_mutex_lock.lock();
...
template <typename PointType> void KD_TREE<PointType>::Update(KD_TREE_NODE *root) {
    ...
    if (left_son_ptr != nullptr && right_son_ptr != nullptr) {
        ...
        if (root->tree_deleted ||
            ...
            tmp_range_x[0] =
                std::min(std::min(left_son_ptr->node_range_x[0], right_son_ptr->node_range_x[0]), root->point.x);
            tmp_range_x[1] =
                std::max(std::max(left_son_ptr->node_range_x[1], right_son_ptr->node_range_x[1]), root->point.x);
            tmp_range_y[0] =
                std::min(std::min(left_son_ptr->node_range_y[0], right_son_ptr->node_range_y[0]), root->point.y);
            tmp_range_y[1] =
                std::max(std::max(left_son_ptr->node_range_y[1], right_son_ptr->node_range_y[1]), root->point.y);
            tmp_range_z[0] =
                std::min(std::min(left_son_ptr->node_range_z[0], right_son_ptr->node_range_z[0]), root->point.z);
            tmp_range_z[1] =
                std::max(std::max(left_son_ptr->node_range_z[1], right_son_ptr->node_range_z[1]), root->point.z);
        } else {
...
template <typename PointType> bool KD_TREE<PointType>::point_cmp_x(PointType a, PointType b) {
    return a.x < b.x;
}
template <typename PointType> bool KD_TREE<PointType>::point_cmp_y(PointType a, PointType b) {
    return a.y < b.y;
}
template <typename PointType> bool KD_TREE<PointType>::point_cmp_z(PointType a, PointType b) {
    return a.z < b.z;
}
...
template <typename T> T MANUAL_Q<T>::front() {
    return q[head];
}

template <typename T> T MANUAL_Q<T>::back() {
    return q[tail];
}
...
template <typename T> bool MANUAL_Q<T>::empty() {
    return is_empty;
}

template <typename T> int MANUAL_Q<T>::size() {
    return counter;
}
